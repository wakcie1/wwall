...
namespace wwall.Bussiness
{
    public class WeChatOAuth
    {
        ...
        public WeChatOAuth(string appid, string appsecret, string redirect_uri, bool IsUserInfo = true)
        {
            this.appid = appid;
        ...
        public void GetAccess_Token()
        {
            ...
            {
                //1.处理跳转
                this.Code = System.Web.HttpContext.Current.Request.QueryString["code"];
                if (string.IsNullOrEmpty(this.Code))
                    throw new Exception("获取code参数失败或用户禁止授权获取基本信息");
                //1.发送获取access_token请求
                string url = GetAccess_TokenUrl();
                string result = CommonHelper.GetWebRequest(url);
                //2.解析相应结果
